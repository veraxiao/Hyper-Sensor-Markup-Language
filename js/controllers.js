...
.controller('ChatsCtrl', function($scope, $compile, $window, mySharedService, uiGmapIsReady) {
  ...
  //});
    $scope.sensorMsg = {'sensor':'',value:0};
...
.controller('indoorMapCtrl', function($scope, $compile, $window, mySharedService, uiGmapIsReady) {    
...
/*    if($scope.locs[0].type == "realtime")
    ...
    var locMarkers = locGroup.selectAll("circle")
                        ...
                        .on("click", function(d,i) {
                            ...
                            if(tooltip.style("display")!="none")
                            {
                                ...
                                if(d.id == "drone")
                                {
                                    $.post(d.src+"/land", function(data){console.info("Drone feedback:", data)});
                                }
                            }
                            else
                            {
                                ...
                                else
                                {
                                    ...
                                    {
                                        $.post(d.src+"/takeoff", function(data){console.info("data:", data)});
                                    }
                                    else
                                    {
    ...
    //listen to sensor messages, if breathing message, update locs['mobileMic']
    $scope.$watch('sensorMsg', function(newValue, oldValue){
        console.info('sensorMsg', newValue, oldValue);
        if(newValue.sensor == 'breathing')
        {
            var index = getIndexbyId('mobileMic', $scope.locs);
            $scope.locs[index].data.pow = newValue.value;
            //$scope.$apply();
            console.info('set sensor value to locs:', index, $scope.locs[index].data.pow);
        }
    });
    
    /*
    //better to define a angular service for all webservice management and use this method as member
    webservices = [];
    var droneWS = {};
    droneWS.id = 'drone';
    droneWS.uri = 'http://localhost:1337/';
    droneWS.RESTs = [{'REST':'up', 'desc':'drone up with specified speed'},
                     {'REST':'down', 'desc':'drone down with fixed speed'}];
    webservices.push(droneWS);
                      
    webservices.getService = function(serviceID){
        for (var i = 0; i < webservices.length; i++)
        if(webservices[i].id == serviceID)
    ...
    */
    ...
    function linear(y, x, slope)
    {
        ...
        var watchVar = '[locs[' + index_x + '].' + x.suffix +',' + slope +']';
        ...
        $scope.$watch(watchVar, function(newValue, oldValue){
            //console.info('watchvar oldValue:',oldValue,';newValue:',newValue);
            
                var URI = $scope.locs[index_y].src + '/upspeed';
                console.info('post URI: ', URI, newValue);
                $.post(URI, {'pow': newValue[0]*newValue[1]}, function(data){
                    console.info("Drone feedback:", data)});
        }, true);        
    }
